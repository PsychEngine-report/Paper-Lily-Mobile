...
namespace LacieEngine.UI
{
	public class InputBindingScreen : Control
	{
		...
		public InputBindingScreen(string action, string actionCaption, Action refreshCallback)
		{
			...
			string caption = ((Inputs.Profiles[Game.Settings.InputProfile].Type != InputProfile.InputType.Keyboard) ? "system.settings.input.pressbutton" : "system.settings.input.presskey");
		...
		public override void _Input(InputEvent @event)
		{
			...
			if (selectedProfile.Type == InputProfile.InputType.Keyboard)
			{
				...
				for (int i = 0; i < blacklist.Length; i++)
				{
					...
					if (key.Scancode == (uint)blacklistedKey)
					{
						return;
					}
				}
				Inputs.Profiles["custom"].UnassignMapping(_action);
				Inputs.Profiles["custom"].UnassignEvent(key);
				Inputs.Profiles["custom"].AddKeyMapping(_action, key.Scancode);
			}
			else if (selectedProfile.Type == InputProfile.InputType.Controller)
			{
				...
				{
					if (!(@event is InputEventKey { Scancode: 16777217u }))
					{
						return;
					}
					ReturnToMenu();
