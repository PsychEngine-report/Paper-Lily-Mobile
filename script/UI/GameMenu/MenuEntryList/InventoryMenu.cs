	...
	public class InventoryMenu : IMenuEntryList
	{
		...
		public InventoryMenu(IMenuEntryContainer container, TitledMenuFrame frame, RichTextLabel description)
		{
			...
			foreach (IItem item in ownedItems)
			{
				if (!item.Hidden)
				{
					_entries.Add(new InventoryEntry(item.Id, Game.Items.GetOwnedAmount(item.Id)));
				}
			}
			foreach (InventoryEntry item2 in _entries)
			{
				Entries.Add(new InventoryMenuEntry(item2, this));
			}
