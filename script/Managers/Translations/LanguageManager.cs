...
namespace LacieEngine.Translations
{
	...
	public class LanguageManager : ILanguageManager, IModule
	{
		...
		private Dictionary<string, LanguageCaptionSet> groupCaptions;
		...
		public void Init()
		{
			...
			baseGroups = new List<string>();
			foreach (string translationFile in GDUtil.ListFilesInPath("res://definitions/captions/", ".csv"))
			{
				baseGroups.Add(translationFile.StripPrefix("res://definitions/captions/").StripSuffix(".csv"));
				foreach (KeyValuePair<string, string> entry in GDUtil.ReadCaptionFile(translationFile, Game.Settings.TranslationBaseLocale))
				{
		...
		public void LoadLanguage(string translationId)
		{
			...
			InitLanguage();
			string locale = translationId;
			TranslationEnabled = locale != Game.Settings.TranslationBaseLocale;
			foreach (KeyValuePair<string, string> entry in baseCaptions)
			{
				AddKey(entry.Key, entry.Value);
			}
			...
			if (TranslationEnabled)
			{
				...
				locale = (currentTranslation.Locale = translationMeta.Locale);
				...
				foreach (string translationFile in GDUtil.ListFilesInPath(translationPath, ".po"))
				{
					...
					foreach (IPOEntry key in po.Catalog.Values)
					{
						...
						else if (!baseGroups.Contains(fileName))
						{
							AddGroupKey(fileName, new LanguageCaption(key.Key.Id, po.Catalog.GetTranslation(key.Key), key.Key.ContextId));
						}
						else
						{
				...
				{
					if (GetCaption(entry2.Value, entry2.Key) != entry2.Value)
					{
						AddKey(entry2.Key, GetCaption(entry2.Value, entry2.Key));
					}
				}
		...
		public string GetCaption(string group, string key, string context)
		{
			if (groupCaptions.ContainsKey(group))
			{
				return groupCaptions[group].Get(key, context);
			}
			return key;
		}
		...
		private void InitLanguage()
		{
			...
			groupCaptions = new Dictionary<string, LanguageCaptionSet>();
		...
		private void AddGroupKey(string group, LanguageCaption caption)
		{
			if (!caption.Str.IsNullOrEmpty())
			{
				if (group.EndsWith("Global"))
				{
					group = "Global";
				}
				if (!groupCaptions.ContainsKey(group))
				{
					groupCaptions[group] = new LanguageCaptionSet();
				}
				groupCaptions[group].AddCaption(caption);
			}
		}
