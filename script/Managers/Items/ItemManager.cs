...
namespace LacieEngine.Items
{
	...
	public class ItemManager : IItemManager, IModule, ITranslatable, IStateOverridable
	{
		...
		public void ApplyTranslationOverrides()
		{
			...
			{
				item.Name = Game.Language.GetCaption(item.Group, item.OriginalName, ItemNameContext(item));
				item.Description = Game.Language.GetCaption(item.Group, item.OriginalDescription, ItemDescriptionContext(item));
			}
		}
