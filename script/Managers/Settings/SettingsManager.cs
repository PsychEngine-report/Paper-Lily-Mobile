...
namespace LacieEngine.Settings
{
	public class SettingsManager
	{
		...
		public bool ShowSkipOnDeath => GetSettingValue<bool, ShowSkipOnDeathSetting>();
		...
		public void LoadSettings(bool fullReload = false)
		{
			...
			_settings[typeof(ShowSkipOnDeathSetting)] = ShowSkipOnDeathSetting.Instance;
		...
		private System.Collections.Generic.Dictionary<string, string[]> GetTranslationRemapsSetting()
		{
			...
			try
			{
				...
				foreach (string k in godotDict.Keys)
				{
					csharpDict[k] = (string[])godotDict[k];
				}
			}
			catch (Exception ex)
			{
				Log.Error("Failed to load Translation Remaps from settings: ", ex.Message);
			}
			return csharpDict;
		}
	}
