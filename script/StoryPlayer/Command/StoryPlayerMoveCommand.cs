...
namespace LacieEngine.StoryPlayer
{
	...
	public class StoryPlayerMoveCommand : StoryPlayerCommand
	{
		...
		private void ChangeRoom(StoryPlayer storyPlayer)
		{
			...
			{
				Game.Room.ChangeRoom(Room, Point, Vector2.Zero, Direction, Time ?? ((float?)null), storyPlayer.Event || Cutscene);
			}
			else
			{
				Game.Room.ChangeRoom(Room, null, Coordinates.Value, Direction, Time ?? ((float?)null), storyPlayer.Event || Cutscene);
			}
		}
	}
}
