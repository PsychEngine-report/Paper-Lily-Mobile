...
namespace LacieEngine.StoryPlayer
{
	public class TextDisplayManager : Node
	{
		...
		public static readonly Regex SpaceTagRegex = new Regex("\\${sp:(\\d+?)}");

		public static readonly Regex FullSpaceTagRegex = new Regex("\\${fsp:(\\d+?)}");

		public static readonly Regex BreakTagRegex = new Regex("\\${br:(\\d+?)}");
		...
		public void SetText(string text)
		{
			if (SpaceTagRegex.IsMatch(text))
			{
				...
				{
					int amountOfSpaces = int.Parse(match.Groups[1].Value);
					text = text.Replace(match.Groups[0].Value, new string(' ', amountOfSpaces));
				}
			}
			if (FullSpaceTagRegex.IsMatch(text))
			{
				foreach (Match match2 in FullSpaceTagRegex.Matches(text))
				{
					int amountOfFullSpaces = int.Parse(match2.Groups[1].Value);
					text = text.Replace(match2.Groups[0].Value, new string('\u3000', amountOfFullSpaces));
				}
			}
			if (BreakTagRegex.IsMatch(text))
			{
				foreach (Match match3 in BreakTagRegex.Matches(text))
				{
					int amountOfBreaks = int.Parse(match3.Groups[1].Value);
					text = text.Replace(match3.Groups[0].Value, new string('\n', amountOfBreaks));
				}
			}
			if (VarTagRegex.IsMatch(text))
			{
				foreach (Match match4 in VarTagRegex.Matches(text))
				{
					string varContent = Game.Variables.GetVariable(match4.Groups[1].Value);
					text = ((!Game.Language.IsCaptionKey(varContent)) ? text.Replace(match4.Groups[0].Value, varContent) : text.Replace(match4.Groups[0].Value, Game.Language.GetCaption(varContent)));
				}
			}
			if (ImageTagRegex.IsMatch(text))
			{
				foreach (Match match5 in ImageTagRegex.Matches(text))
				{
					text = text.Replace(match5.Groups[0].Value, "[img=15]res://assets/img/ui/" + match5.Groups[1].Value + ".png[/img]") + " ";
				}
			}
			if (ItemTagRegex.IsMatch(text))
			{
				foreach (Match match6 in ItemTagRegex.Matches(text))
				{
					text = text.Replace(match6.Groups[0].Value, "[font=res://resources/font/image_valign.tres][img=20]res://assets/sprite/common/item/" + Game.Items.Get(match6.Groups[1].Value).Icon + ".png[/img][/font][color=aqua]" + Game.Items.Get(match6.Groups[1].Value).Name + "[/color]") + " ";
				}
			}
			IsTextNode = true;
