	...
	public class Chair : Node2D, IEditorArea
	{
		public class ActualSitSurface : StaticBody2D, IActionObject
		{
			...
			public void Activate()
			{
				...
				{
					float minPos = base.GlobalPosition.x - _surface.Area.x / 2f + (float)_surface.WideMargin;
					float maxPos = base.GlobalPosition.x + _surface.Area.x / 2f - (float)_surface.WideMargin;
				...
				{
					float minPos2 = base.GlobalPosition.y - _surface.Area.y / 2f + (float)_surface.WideMargin;
					float maxPos2 = base.GlobalPosition.y + _surface.Area.y / 2f - (float)_surface.WideMargin;
			...
			public void Deactivate()
			{
				...
				{
					int newX = (int)(base.GlobalPosition.x + surfaceDirection.ToVector().x * (_surface.Area.x / 2f + 10f));
					int newY = (int)(_surface.Wide ? ((float)(int)_playerOriginalPosition.y) : base.GlobalPosition.y);
					newPlayerGlobalPosition = new Vector2(newX, newY);
				}
				else if (surfaceDirection.HasY())
				{
					int newX2 = (int)(_surface.Wide ? _playerOriginalPosition.x : base.GlobalPosition.x);
					int newY2 = (int)(base.GlobalPosition.y + surfaceDirection.ToVector().y * (_surface.Area.y / 2f + 10f));
