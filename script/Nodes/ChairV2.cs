		...
		public class ActualSitSurface : StaticBody2D, IActionObject
		{
			...
			public void Activate()
			{
				if (_surface.Enabled)
				{
					...
					{
						float minPos = base.GlobalPosition.x - _surface.Area.x / 2f + (float)_surface.WideMarginX;
						float maxPos = base.GlobalPosition.x + _surface.Area.x / 2f - (float)_surface.WideMarginX;
						_playerOriginalPosition.x = Math.Max(minPos, _playerOriginalPosition.x);
						_playerOriginalPosition.x = Math.Min(maxPos, _playerOriginalPosition.x);
					...
					{
						float minPos2 = base.GlobalPosition.y - _surface.Area.y / 2f + (float)_surface.WideMarginY;
						float maxPos2 = base.GlobalPosition.y + _surface.Area.y / 2f - (float)_surface.WideMarginY;
						_playerOriginalPosition.y = Math.Max(minPos2, _playerOriginalPosition.y);
						_playerOriginalPosition.y = Math.Min(maxPos2, _playerOriginalPosition.y);
					...
					Game.StoryPlayer.Connect("DialogueStarted", this, "Deactivate");
			...
			public void Deactivate()
			{
				...
				{
					int newX = (int)(_surface.WideX ? _playerOriginalPosition.x : base.GlobalPosition.x);
					int newY = (int)(base.GlobalPosition.y + _curSurfaceDirection.ToVector().y * (_surface.Area.y / 2f + 10f));
					newPlayerGlobalPosition = new Vector2(newX, newY);
				}
				else if (_curSurfaceDirection.HasX())
				{
					int newX2 = (int)(base.GlobalPosition.x + _curSurfaceDirection.ToVector().x * (_surface.Area.x / 2f + 10f));
					int newY2 = (int)(_surface.WideY ? ((float)(int)_playerOriginalPosition.y) : base.GlobalPosition.y);
