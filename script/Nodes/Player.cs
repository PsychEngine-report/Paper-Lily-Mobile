...
namespace LacieEngine.Nodes
{
	public class Player : WalkingCharacter, IPlayer, IPhysicsInterpolated, ITurnable, IWalker, IReflectable, IFollowable
	{
		...
		public EventTrigger GetInteractingNode()
		{
			...
			foreach (Node2D overlappingArea in nFloorDetection.GetOverlappingAreas())
			{
				if (overlappingArea.GetParent() is EventTrigger { Trigger: EventTrigger.ETrigger.Action } et && Game.Events.HasMapping(et.Name, SpriteDirectionTemp))
				{
					nodes.Add(et);
				}
			}
			foreach (Node2D overlappingBody in nThinInteractions.GetOverlappingBodies())
			{
				if (overlappingBody.GetParent() is EventTrigger { Trigger: EventTrigger.ETrigger.Action } et2 && Game.Events.HasMapping(et2.Name, SpriteDirectionTemp))
				{
					nodes.Add(et2);
				}
			}
			...
			if (nodes.IsEmpty())
			{
				...
				{
					if (overlappingBody2.GetParent() is EventTrigger { Trigger: EventTrigger.ETrigger.Action } et3 && Game.Events.HasMapping(et3.Name, SpriteDirectionTemp))
					{
						nodes.Add(et3);
					}
				}
		...
		public EventTrigger GetItemInteractingNode(string itemId)
		{
			...
			foreach (Node2D overlappingArea in nFloorDetection.GetOverlappingAreas())
			{
				if (overlappingArea.GetParent() is EventTrigger et && (et.Trigger == EventTrigger.ETrigger.Action || et.Trigger == EventTrigger.ETrigger.Item) && Game.Events.HasItemObjectMapping(itemId, et.Name))
				{
					nodes.Add(et);
				}
			}
			...
			if (nodes.IsEmpty())
			{
				...
				foreach (Node2D overlappingBody2 in nInteractions.GetOverlappingBodies())
				{
					if (overlappingBody2.GetParent() is EventTrigger et3 && (et3.Trigger == EventTrigger.ETrigger.Action || et3.Trigger == EventTrigger.ETrigger.Item) && Game.Events.HasItemObjectMapping(itemId, et3.Name))
					{
						nodes.Add(et3);
					}
				}
		...
		{
			if (!AutoMovement && area.GetParent() is EventTrigger { Trigger: EventTrigger.ETrigger.Touch } et && Game.Events.HasInstantMapping(et.Name))
			{
