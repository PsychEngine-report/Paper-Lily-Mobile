...
namespace LacieEngine.Minigames
{
	public class Ch1MinigameBattleMkPt2 : Ch1MinigameBattleMk
	{
		...
		public override async Task TransitionOut()
		{
			if (base._hp > 0)
			{
				Control overlay = UIUtil.CreateOverlay(Colors.White);
				GetParent().AddChild(overlay);
				FadeInAnimation transition = new FadeInAnimation(overlay, 4f);
			...
			else
			{
				...
				Control overlay2 = UIUtil.CreateOverlay(Colors.Black);
