...
namespace LacieEngine.Minigames
{
	public class Ch1MinigameBattleRedBridge3 : Ch1MinigameBattleRed
	{
		...
		{
			base._Process(delta);
			foreach (double triggerTime in PlayerPanelAttacks)
			{
				if (triggerTime - 0.800000011920929 - base._elapsed <= 0.0)
				{
					SpikesOnPanel(base._playerX, base._playerY, triggerTime);
				}
			}
			PlayerPanelAttacks.RemoveAll((double num) => num - 0.800000011920929 - base._elapsed <= 0.0);
		}
	}
}
