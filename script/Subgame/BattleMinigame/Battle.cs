...
namespace LacieEngine.Minigames
{
	public class Battle : Control, IMinigame
	{
		...
		public override void _Process(float delta)
		{
			...
			foreach (BattleSound sound in _currentSounds)
			{
				if (_elapsed >= (double)sound.PlayTime)
				{
					Game.Audio.PlaySfx(sound.Sfx, sound.Volume);
					sound.Played = true;
				}
			}
			for (int i = 0; i < Width; i++)
			{
				...
				{
					if (Cells[i, j].Telegraphing)
					{
						ProcessTelegraph(i, j);
					}
					if (Cells[i, j].Damaging)
					{
						ProcessDamage(i, j);
					}
				}
			...
			_currentSounds.RemoveAll((BattleSound battleSound) => battleSound.Played);
		...
		private void ProcessDamage(int x, int y)
		{
			...
			if (!Invincible && _playerX == x && _playerY == y && _invulnerabilityTimer <= 0.0)
			{
				_hp--;
