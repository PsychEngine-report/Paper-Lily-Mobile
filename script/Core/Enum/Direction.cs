...
namespace LacieEngine.Core
{
	...
	public class Direction : ISerializable
	{
		...
		public static Direction FromVectorAnalog(Vector2 direction)
		{
			if (direction.x < 0f && direction.y < 0f)
			{
				return Decide1of(direction, Up, Left, UpLeft);
			}
			if (direction.x < 0f && direction.y > 0f)
			{
				return Decide1of(direction, Down, Left, DownLeft);
			}
			if (direction.x > 0f && direction.y < 0f)
			{
				return Decide1of(direction, Up, Right, UpRight);
			}
			if (direction.x > 0f && direction.y > 0f)
			{
				return Decide1of(direction, Down, Right, DownRight);
			}
			if (direction.x < 0f)
			{
				return Left;
			...
			return None;
			static Direction Decide1of(Vector2 vector, Direction dOne, Direction dTwo, Direction dThree)
			{
				float one = vector.Project(dOne.ToVector()).Length();
				float two = vector.Project(dTwo.ToVector()).Length();
				float three = vector.Project(dThree.ToVector()).Length();
				if (one > two)
				{
					if (one > three)
					{
