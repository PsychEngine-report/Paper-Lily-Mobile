...
namespace LacieEngine.Core
{
	public class SaveFileInformation
	{
		...
		public static bool CanLoadRetrySave()
		{
			...
			SaveFileInformation retryInfo = GetSaveFileInformation("retrysave");
			if (!retryInfo.CanPlay())
			{
				return false;
			}
			foreach (string item in GDUtil.ListFilesInPath("user://save/", "slot", ".sav"))
			{
				if (GetSaveFileInformation(GDUtil.GetFileNameFromPath(item, removeExtension: true)).Date > retryInfo.Date)
				{
					return false;
				}
			}
		...
		public bool CanPlay()
		{
			if (!Empty && !Corrupted)
			{
				if (Event.IsNullOrEmpty() || !Game.Events.Exists(Event))
				{
					if (Event.IsNullOrEmpty())
					{
						return GDUtil.FileExists("res://resources/nodes/rooms/" + Room + ".tscn");
					}
					return false;
				}
				return true;
