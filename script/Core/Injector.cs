...
namespace LacieEngine.Core
{
	public class Injector
	{
		...
		public static void Init()
		{
			...
			{
				Type[] ınterfaces = type.GetInterfaces();
				foreach (Type @interface in ınterfaces)
				{
					if (@interface.GetCustomAttributes(typeof(InjectableInterface), inherit: true).Length != 0)
					{
						if (!_allImpls.ContainsKey(@interface))
						{
							_allImpls[@interface] = new List<Type>();
						}
						_allImpls[@interface].Add(type);
					}
				}
			}
			foreach (Type interface2 in _allImpls.Keys)
			{
				...
				{
					int priority = implementation.GetCustomAttribute<Injectable>().priority;
					string condition = implementation.GetCustomAttribute<Injectable>().condition;
				...
				{
					if (implementation2 != selectedImpl)
					{
						bannedImplementations.Add(implementation2);
					}
				}
			}
			...
			foreach (Type interface3 in DrkieUtil.GetTypesWithAttribute(typeof(InjectableInterface)))
			{
				if (!_implementations.ContainsKey(interface3))
				{
					Log.Warn("Interface ", interface3.Name, " has no implementations!");
				}
			}
		}
